...
const HeaderHome = ({state,handleReset,handleOrder}) => {
  ...
  return (
      <header className='row m-auto w-100 justify-content-center text-center gap-1 p-1 py-3'>
          ...
          <button onClick={handleHadleOrder} className={(state.length === 4 ? "" : "disabled ") + 'btn btn-primary order-sm-3 col col-sm-2 order-3 p-1'}>Order</button>
      </header>
  )
}
